...
/* USER CODE BEGIN 1 */
void HAL_HRTIM_FaultCallback(HRTIM_HandleTypeDef *hhrtim,
                              uint32_t TimerIdx,
                              uint32_t FaultSource)
{
  if(FaultSource == HRTIM_FAULT_3)
  {
    // 1. PWM 출력은 자동으로 비활성화됨 (하드웨어에서 처리)

    // 2. 게이트 드라이버 비활성화 (안전을 위해)
    HAL_GPIO_WritePin(PWM_EN_GPIO_Port, PWM_EN_Pin, GPIO_PIN_RESET);

    HardWareFault();

    // 3. 에러 플래그 설정 (사용자 변수 - 필요시)
    // fault_error_flag = 1;
